...
# Todas as variantes são 4x4 para consistência
PIECES = {
    'I': [
        ["....",
         "1111",
...
def shape_to_coords(matrix):
    ...
    for y, row in enumerate(matrix):
        # Garante que cada linha é tratada como string (comprimentos iguais esperados)
...
class Board:
    ...
    def is_valid_position(self, piece, offset_x=0, offset_y=0):
        for x, y in piece.get_coords():
            ...
            if y_off >= ROWS:
                return False
            # células acima do topo são permitidas (y_off < 0)
    ...
    def remove_full_lines(self):
        new_grid = [row for row in self.grid if '' in row]  # mantém linhas incompletas
...
class Game:
    def __init__(self):
        ...
        # Controle de queda baseado em tempo (mais confiável que contar frames)
        self.drop_interval_ms = 500  # tempo entre quedas em ms (ajuste para dificuldade)
    ...
    def rotate_piece(self):
        self.piece.rotate()
        # Tenta ajuste simples (kick) para permitir rotações na borda
        ...
        for kx, ky in kicks:
            if self.board.is_valid_position(self.piece, offset_x=kx, offset_y=ky):
                ...
                self.piece.y += ky
                return
        # se nenhum kick funcionou, desfaz a rotação
...
def draw_next_piece(screen, piece):
    # desenha o próximo no painel lateral (usa a variante atual rot=0 para pré-visualizar)
